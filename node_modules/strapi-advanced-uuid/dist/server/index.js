...
const bootstrap = ({ strapi: strapi2 }) => {
  const { contentTypes: contentTypes2 } = strapi2;
  const models = Object.keys(contentTypes2).reduce((acc, key) => {
    const contentType = contentTypes2[key];
  ...
  if (strapi2.db) {
    strapi2.db.lifecycles.subscribe({
      ...
      beforeCreate(event) {
        strapi2.plugin(PLUGIN_ID).service("service").handleCRUDOperation(event);
      },
      beforeUpdate(event) {
        strapi2.plugin(PLUGIN_ID).service("service").handleCRUDOperation(event);
      }
...
const controller = ({ strapi: strapi2 }) => ({
  index(ctx) {
    ctx.body = strapi2.plugin("strapi-advanced-uuid").service("service").getWelcomeMessage();
  }
});
const controllers = {
  controller
...
var stringify$4 = (ast, options = {}) => {
  const stringify2 = (node, parent = {}) => {
    const invalidBlock = options.escapeInvalid && utils$8.isInvalidBrace(parent);
...
const compile$1 = (ast, options = {}) => {
  const walk = (node, parent = {}) => {
    const invalidBlock = utils$7.isInvalidBrace(parent);
    const invalidNode = node.invalid === true && options.escapeInvalid === true;
...
const append = (queue = "", stash = "", enclose = false) => {
  ...
  if (!queue.length) {
    return enclose ? utils$6.flatten(stash).map((ele) => `{${ele}}`) : stash;
  }
...
const expand$1 = (ast, options = {}) => {
  ...
  const walk = (node, parent = {}) => {
    ...
    if (node.nodes && node.ranges > 0) {
      ...
      if (utils$6.exceedsLimit(...args, options.step, rangeLimit)) {
    ...
    const enclose = utils$6.encloseBrace(node);
  ...
  return utils$6.flatten(walk(ast));
};
...
const parse$2 = (input, options) => {
  ...
  const win32 = utils$3.isWindows(options);
...
picomatch$2.matchBase = (input, glob, options, posix = utils$2.isWindows(options)) => {
...
let YupValidationError$1 = class YupValidationError extends ValidationError2 {
...
const errors = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  ...
  YupValidationError: YupValidationError$1
...
const isValidUUIDValue = (uuidFormat, value) => {
  const isValidValue2 = uuidFormat ? validateUUID(uuidFormat, value) : uuid.validate(value);
  if (value && !isValidValue2) {
    return false;
  }
  return true;
};
const { YupValidationError: YupValidationError2 } = errors;
const service = ({ strapi: strapi2 }) => ({
  getWelcomeMessage() {
    return "Welcome to Strapi 🚀";
  },
  handleCRUDOperation(event) {
    const errorMessages = {
      inner: []
    };
    Object.keys(event.model.attributes).forEach((attribute) => {
      if (event.model.attributes[attribute].customField === "plugin::strapi-advanced-uuid.uuid") {
        ...
        const options = event.model.attributes ? event.model.attributes[attribute]["options"] : null;
        ...
        if (!disableAutoFill || initialValue) {
          if (!isValidUUIDValue(uuidFormat, event.params.data[attribute])) {
            errorMessages.inner.push({
              ...
              path: attribute,
              // Name of field we want to show input validation on
    ...
    if (errorMessages.inner.length > 0) {
      throw new YupValidationError2(errorMessages, "You have some issues");
    }
  }
});
const services = {
